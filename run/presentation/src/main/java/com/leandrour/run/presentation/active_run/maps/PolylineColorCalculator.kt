package com.leandrour.run.presentation.active_run.maps
...
object PolylineColorCalculator {
    ...
    fun locationsToColor(location1: LocationTimestamp, location2: LocationTimestamp): Color {
        ...
        val timeDiff = abs((location2.durationTimestamp - location1.durationTimestamp).inWholeHours)
    ...
    private fun interpolateColor(
        ...
        val colorInt = if (ratio <= 0.5) {
            val minRatio = ratio / 0.5
            ColorUtils.blendARGB(colorStart.toArgb(), colorMid.toArgb(), minRatio.toFloat())
        } else {
            val minToEndRatio = (ratio - 0.5) / 0.5
            ColorUtils.blendARGB(colorMid.toArgb(), colorEnd.toArgb(), minToEndRatio.toFloat())
        }
